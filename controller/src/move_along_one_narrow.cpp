...
#define load_data_FC_1 "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_w31.txt"  // wei for max speed = 50.0 rad/s
...
////////////////////////////////////////////  Constructor  //////////////////////////////////////////////	
		controller(){
    		...
    		pub=n.advertise<geometry_msgs::Twist>("/cmd_vel",3000); //cmd_vel_2
	  ...
	  return laser_min;
		...
		{	
			...
			if(decision_right == 1)
			{
				...
				read_4 = laser_temp[100] ;
				read_5 = laser_temp[95] ;
				read_6 = laser_temp[95] ;

				for(size_t i = 155; i <= 179; i++)//25
				{	if(read_1>laser_temp[i])
					{
						read_1=std::min(laser_temp[i],far);
						input_deg[1] = i;
...
//////////////////////////////////  Rescale sensor data for fuzzy  /////////////////////////////////////////
		...
		//感測器讀值最大為5m (參閱副程式: get_sensor ) ，因此乘上1.0代表把所有讀值壓縮到 0~1 之間。  
			if(is_narrow_passage)
			{
				if(decision_right)
				{
					in[1] = sick_1[jj] - (0.15 / cos((input_deg[1] - 90) * M_PI / 180.0)); 
					in[2] = sick_2[jj] - (0.15 / cos((input_deg[2] - 90) * M_PI / 180.0));  
					in[3] = sick_3[jj] - (0.15 / cos((input_deg[3] - 90) * M_PI / 180.0));  
					in[4] = sick_4[jj] - (0.15 / cos((input_deg[4] - 90) * M_PI / 180.0));
					printf("\ninput倍數轉換過, right wall\n");
				}
				else if(decision_left)
				{
					in[1] = sick_1[jj] - (0.15 / cos((270 - input_deg[1]) * M_PI / 180.0)); 
					in[2] = sick_2[jj] - (0.15 / cos((270 - input_deg[2]) * M_PI / 180.0));  
					in[3] = sick_3[jj] - (0.15 / cos((270 - input_deg[3]) * M_PI / 180.0));  
					in[4] = sick_4[jj] - (0.15 / cos((270 - input_deg[4]) * M_PI / 180.0));
