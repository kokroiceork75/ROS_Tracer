...
// #define load_data_FC "controller/nsga_v1/temp_w1.txt"  // wei change for max speed = 15.6 rad/s
#define load_data_FC_1 "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_w12.txt"  // wei for max speed = 50.0 rad/s
...
int input_deg[_in_varl + 1]; // for narrow wall
   ...
   {  
	  ...
	  return laser_min;
		...
		void callback(const sensor_msgs::LaserScan::ConstPtr& scan) //wei change,not sure
		{
           ...
           left_min=minimun(205,269,k);
		   right_min=minimun(90,155,k);
		   stright_min=minimun(165,195,k);
		}
		void decision_(int j)
		{	
			check_narrow_passage(); // check whether narrow passage or not
			...
			{	
				...
				read_3 = laser_temp[245] ;
				read_4 = laser_temp[315] ;
				read_5 = laser_temp[265] ;
				read_6 = laser_temp[265] ;
				for(size_t i = 180; i <= 210; i++)//25
			...
			if(decision_right == 1)
			{
				...
				read_3 = laser_temp[110] ;
				read_4 = laser_temp[45] ;
				read_5 = laser_temp[95] ;
				read_6 = laser_temp[95] ;

				for(size_t i = 150; i <= 179; i++)//25
				{	if(read_1>laser_temp[i])
					{
						read_1=std::min(laser_temp[i],far);
						input_deg[1] = i;
...
///////////////////////////////////  Open the file from leaning ///////////////////////////////
		...
		void test_open()
		{
			...
			// **根據狹窄通道模式切換不同的控制器檔案**
			const char *selected_FC = is_narrow_passage ? load_data_FC_1 : load_data_FC_1;
			printf("Selected fuzzy controller file: %s\n", selected_FC);
			if ((fnoise1 = fopen(selected_FC, "r")) == NULL) 
			{
				printf("Error: Unable to open fuzzy controller file: %s\n", selected_FC);
				exit(1);
			}
...
//////////////////////////////////  Rescale sensor data for fuzzy  /////////////////////////////////////////
		...
		//感測器讀值最大為5m (參閱副程式: get_sensor ) ，因此乘上1.0代表把所有讀值壓縮到 0~1 之間。  
			if(is_narrow_passage)
			{
				if(decision_right)
				{
					in[1] = sick_1[jj] - (reduce_dis / cos((input_deg[1] - 90) * M_PI / 180.0)); 
					in[2] = sick_2[jj] - (reduce_dis / cos((input_deg[2] - 90) * M_PI / 180.0));  
					in[3] = sick_3[jj] - (reduce_dis / cos((input_deg[3] - 90) * M_PI / 180.0));  
					in[4] = sick_4[jj] - (reduce_dis / cos((input_deg[4] - 90) * M_PI / 180.0));
					printf("\ninput倍數轉換過, right wall\n");
					printf("+++++++++++++++++++++++++++\n");
				}
				else if(decision_left)
				{
					in[1] = sick_1[jj] - (reduce_dis / cos((270 - input_deg[1]) * M_PI / 180.0)); 
					in[2] = sick_2[jj] - (reduce_dis / cos((270 - input_deg[2]) * M_PI / 180.0));  
					in[3] = sick_3[jj] - (reduce_dis / cos((270 - input_deg[3]) * M_PI / 180.0));  
					in[4] = sick_4[jj] - (reduce_dis / cos((270 - input_deg[4]) * M_PI / 180.0));
					printf("\ninput倍數轉換過, left wall\n");
					printf("+++++++++++++++++++++++++++\n");
				}
			}
			else
			{
				...
				in[3] = sick_3[jj];  
				in[4] = sick_4[jj];
				printf("\n\nnot in narrow passage, is_passage = %d\n", is_narrow_passage);
				printf("+++++++++++++++++++++++++++++\n");
			}
			printf("scale input 1 === %f, degree = %d\n" , 		in[1], input_deg[1]); 
			printf("scale input 2 === %f, degree = %d\n" , 		in[2], input_deg[2]); 
			printf("scale input 3 === %f, degree = %d\n" , 		in[3], input_deg[3]); 
			printf("scale input 4 === %f, degree = %d\n" , 		in[4], input_deg[4]); 
...
//////////////////////////////////  Stop the car  ///////////////////////////////////////
		...
		void check_narrow_passage()
		{
			int min_allow_width = 0.57 + 0.25;
			int narrow_width = right_min + left_min;
			if(narrow_width <= min_allow_width)
