...
class amcl_pose_sub_pub_1 
{
  ...
  public:
	...
	{ 
		...
		// 檢查當前路徑是否已初始化（第一次收到路徑時直接採用）
		if (ik == 0 && local_target_x[0] == 0 && local_target_y[0] == 0) {
			for (std::vector<geometry_msgs::PoseStamped>::const_iterator it = data.begin(); it != data.end(); ++it) {
				local_target_x[i] = msg->poses[i].pose.position.x;
				local_target_y[i] = msg->poses[i].pose.position.y;
				tf2::Quaternion g;
			...
			printf("第一次收到路徑，已初始化當前路徑，ik = %d\n", ik);
			return;
		}

		// 計算當前路徑中下一個目標的方向與車頭方向的角度差
		double next_target_x = local_target_x[ik];
		double next_target_y = local_target_y[ik];
		double target_angle = atan2(next_target_y - amcl_y, next_target_x - amcl_x);
		double angle_diff = fabs(target_angle - amcl_z);
		if (angle_diff > M_PI) angle_diff = 2 * M_PI - angle_diff;

		// 角度差 ≥ 45 度，保留當前路徑
		if (angle_diff >= M_PI / 4) {
			printf("++++++++++當前路徑角度差 %.2f 度 ≥ 45 度，保留當前路徑 (ik = %d)++++++++++++++\n", angle_diff * 180 / M_PI, ik);
			printf("no change path  ## -. -\n");
			return;
		}

		// 角度差 < 45 度，更新為新路徑
		printf("##############當前路徑角度差 %.2f 度 < 45 度，更新為新路徑##############\n", angle_diff * 180 / M_PI);
		printf("~~~  \\^3^//\n");
		i = 0;
		double temp_target_x[max_step + 1], temp_target_y[max_step + 1], temp_target_z[max_step + 1];
		int temp_path_size = 0;
		for (std::vector<geometry_msgs::PoseStamped>::const_iterator it = data.begin(); it != data.end(); ++it) {
			temp_target_x[i] = msg->poses[i].pose.position.x;
			temp_target_y[i] = msg->poses[i].pose.position.y;
			tf2::Quaternion g;
			tf2::convert(msg->poses[i].pose.orientation, g);
			tf2::Matrix3x3(g).getRPY(roll_g, pitch_g, yaw_g);
			temp_target_z[i] = yaw_g;
			local_rms = sqrt(pow(amcl_x - temp_target_x[i], 2) + pow(amcl_y - temp_target_y[i], 2));
			if (local_rms <= 1 - 0.5 * fabs(error_orientation_z) / (M_PI / 2)) {
				ik = i; // 選擇距離較遠的目標點
			}
			i++;
		...
		// 更新 local_target_* 為新路徑
		for (int j = 0; j < temp_path_size; j++) {
			local_target_x[j] = temp_target_x[j];
			local_target_y[j] = temp_target_y[j];
			local_target_z[j] = temp_target_z[j];
		}
		if (temp_path_size <= 20) {  
			ik = temp_path_size;
			local_target_x[temp_path_size] = goal_x;
			local_target_y[temp_path_size] = goal_y; 
			local_target_z[temp_path_size] = goal_z;
		}
		printf("新路徑已採用，ik = %d\n", ik);
	}
    void coordinate_transform(int jj) 
	{
		...
		// 如果接近當前目標，移動到下一個點，放寬閾值到 0.5
		double local_dist = sqrt(pow(local_error_x, 2) + pow(local_error_y, 2));
		if (local_dist < 0.5 && ik < max_step) { // 從 0.1 改為 0.5
			ik++;
			printf("接近目標，移動到下一個點，ik = %d\n", ik);
		}
	}
