...
#define load_data_FC_1 "controller/along_wall_controller/temp_w12.txt"  // wei for max speed = 50.0 rad/s
#define load_data_FC_2 "controller/along_wall_controller/temp_narrow.txt"  // wei for narrow wall 
   ...
   {  
	  ...
	  return laser_min;
		...
		{	
			...
			if (stright_min <= 0.45 || left_min <= 0.57 || right_min <= 0.57){
				if (left_min <= 0.57)
				{
					decision_left = 1;
					decision_right= 0;
...
///////////////////////////////////  Open the file from leaning ///////////////////////////////
		...
		void test_open()
		{
			...
			fclose(fnoise0);
			if((fnoise1=fopen(load_data_FC_1,"r"))==NULL)
			{
				printf("Nothing2\n");
				exit(1);
			}
