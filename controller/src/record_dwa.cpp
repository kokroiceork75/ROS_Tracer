...
int main(int argc, char** argv) {
    ...
    dataFile.open(SAVE_PATH + "data_dwa.txt");
    if (!dataFile) {
        ROS_ERROR("Failed to open file for writing. Please check the path: %s", SAVE_PATH.c_str());
        return -1;
    }
    
    // 更新文件頭，移除 controller_flag
    dataFile << "time\tx\ty\torientation_x\torientation_y\torientation_z\torientation_w\tlinear_vel\tangular_vel" << std::endl;
