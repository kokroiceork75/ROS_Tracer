...
#define dis_to_wall 0.3
#define radius 0.03

#define load_data_clu "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/10.txt"
#define load_data_FC "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_w45.txt"  // wei for max speed = 50.0 rad/s
   ...
   {  
	  ...
	  return laser_min;
		...
		void callback(const sensor_msgs::LaserScan::ConstPtr& scan)
		{
			...
			left_min = minimun(210,269,k); // 270 seems zero
		...
		void decision_(int j){	
			...
			if (stright_min<=0.4 || left_min<=0.6 || right_min<=0.6){
				if (left_min<=0.6){
					decision_left = 1;
					decision_right= 0;
				}
				else if (right_min<=0.6){
					decision_left = 0;
					decision_right= 1;
				
				}
			    else if (left_min<right_min && left_min <0.6){
					decision_left = 1;
					decision_right= 0;
				}
				else if (left_min>right_min && right_min<0.6){
					decision_left = 0;
					decision_right= 1;
				}
			}
		    ...
		    else if(right_min < 0.6 && laser_temp[269] > 0.6 )// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 1;
				decision_left  = 0;
			
			}
			else if(left_min <0.6 && laser_temp[90]> 0.6)// && laser_temp[270] < 0.7 )//&& decision_right == 0 )
		    {
				decision_left = 1;
				decision_right= 0;
...
////////////////////////////////////  Output cmd_velocity  ////////////////////////////////////
		void test_run(int j){
			...
			v1 = out_y[1] * radius * v_adjust;
			v2 = out_y[2] * radius * v_adjust;
			}
			if(decision_right == 1 ){
			 v1 = out_y[2] * radius * v_adjust;
			 v2 = out_y[1] * radius * v_adjust;
			}
		
			msg.linear.x =(v1+v2)/2 ;
			if((v1-v2)/dis > 7. )  msg.angular.z = 7.; 
			else if ((v1-v2)/dis <-7.) msg.angular.z =-7.;
			else
...
int main(int argc,char ** argv)
{
	...
	while(counts < max_step && ros::ok())
	{
	   ...
	   ros::spinOnce();
      ros::Duration(0.01).sleep(); // original An's 100 Hz
