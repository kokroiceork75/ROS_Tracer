...
// #define save_data_path1    "position_random/20240821/position_all_fuzzy_pid-1.txt"  //整條路徑
// #define save_along_wall    "position_random/20240821/position_along_wall_fuzzy_pid-1.txt" //沿牆
// #define save_search_target "position_random/20240821/posititon_search_target_fuzzy_pid-1.txt" //尋標
// #define save_distance      "position_random/20240821/distance-1.txt"  //左/右/前測距離
// #define save_velocity      "position_random/20240821/velocity-1.txt" 
// #define save_velocity_every "position_random/20240821/velocity-every-1.txt"
// #define save_every_error   "position_random/20240821/dis_error-1.txt"
...
public:
   ...
   void lasercallback(const sensor_msgs::LaserScan::ConstPtr& scan)
   {
      ...
      if (!std::isinf(laser_temp_scan[i]))
      {
         ...
         right_min=minimun(90,155,k);
		   straight_min=minimun(155,205,k);
  ...
  double minimun(int i, int j ,int &k)
   ...
   void amcl_Callback(const::move_base_msgs::MoveBaseActionFeedback::ConstPtr & amcl){
      ...
      double distance = sqrt(pow(amcl_position_x - target_position_x, 2) + pow(amcl_position_y - target_position_y, 2));
      if (distance < 0.05 && !this->reached_goal) { // **0.05 公尺內視為到達**
