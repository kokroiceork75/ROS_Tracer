...
#define dis_to_wall 0.30
#define radius 0.03

#define load_data_clu "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/10.txt"
#define load_data_FC "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_w35.txt"  // wei for max speed = 50.0 rad/s
   ...
   {  
	  ...
	  return laser_min;
		...
		void callback(const sensor_msgs::LaserScan::ConstPtr& scan)
		{
			...
			left_min = minimun(205,269,k); // 270 seems zero
			right_min = minimun(90,155,k);
			stright_min = minimun(155,205,k);
		}
		void decision_(int j){	
			...
			if (stright_min<=f_along_dis || left_min<=s_slong_dis || right_min<=s_slong_dis)
			{
			    ...
			    else if (left_min<right_min && left_min <s_slong_dis){
					decision_left = 1;
					decision_right= 0;
				}
				else if (left_min>right_min && right_min<s_slong_dis){
					decision_left = 0;
					decision_right= 1;
				}
			}
		    ...
		    else if(right_min < s_slong_dis && laser_temp[269] > s_slong_dis)// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 1;
				decision_left  = 0;
			
			}
			else if(left_min < s_slong_dis && laser_temp[90]> s_slong_dis)// && laser_temp[270] < 0.7 )//&& decision_right == 0 )
		    {
				decision_left = 1;
				decision_right= 0;
			...
			if(decision_right == 1)
			{
				printf("mode: RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR\n");
...
////////////////////////////////////  Output cmd_velocity  ////////////////////////////////////
		void test_run(int j){
			...
			v1 = out_y[1] * radius * 0.7;
			v2 = out_y[2] * radius * 0.7;
			}
			if(decision_right == 1 ){
			 v1 = out_y[2] * radius * 0.7;
			 v2 = out_y[1] * radius * 0.7;
			}
