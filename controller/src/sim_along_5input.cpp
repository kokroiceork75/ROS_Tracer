//     Tracer 5 input move along wall //
#include <sstream>
#include <iostream>
#include <algorithm>
#include "ros/ros.h"
   ...
   {  
	  ...
	  return laser_min;
		...
		void decision_(int j){	
			...
			{	if(read_7>laser_temp[i])
				read_7=std::min(laser_temp[i],far);
			}
			//printf("%d\n",4);
...
////////////////////////////////////  Output cmd_velocity  ////////////////////////////////////
		void test_run(int j){
			...
			if(decision_left == 1 ){
			v1 = out_y[1] * r * 1;  // 0.03 is radius of wheel
			v2 = out_y[2] * r * 1;
			}
			if(decision_right == 1 ){
			 v1 = out_y[2] * r * 1;
			...
			msg.linear.x =(v1+v2)/2 ;
			if((v1-v2)/dis > 1. )  msg.angular.z = 1.;  // avoid that turn around too fast
...
///////////////////////////////////  Open the file from leaning ///////////////////////////////
		...
		void publisher_info(int jj ) //印出訊息並傳遞數值給其他程式從pub1~pub10; notice: two "info1" in this code are different
  		{        
...
//////////////////////////////////  Rescale sensor data for fuzzy  /////////////////////////////////////////
		...
		//感測器讀值最大為5m (參閱副程式: get_sensor ) ，因此乘上1.0代表把所有讀值壓縮到 0~1 之間。  
			...
			in[5] = sick_5[jj] * _input_scale ;//平移再normalize//2017_8_2
