...
#define load_data_FC "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_w12.txt"  // wei for max speed = 50.0 rad/s
// #define load_data_FC_2 "/home/user/wei_ws/src/controller/src/fuzzy_controller/along_wall_controller/temp_narrow-1.txt"  // wei for narrow wall 
...
// #define save_every_error "controller/20240821/dis_error.txt" 
...
////////////////////////////////////////////  Constructor  //////////////////////////////////////////////	
		controller(){
		    ...
		    //sub=n.subscribe("/p3dx/laser/scan",3000,&controller::callback,this);
...
void Final_Goal(const geometry_msgs::PoseStamped::ConstPtr & F_goal) //全域目標的X Y Z 的數值
  {
    ...
    if( orientation_z1 <=-1.5707 && orientation_z1>=-3.14159 )
	    orientation_z1= (1.5 * M_PI) + orientation_z1; 
    else
   ...
   {  
	  ...
	  return laser_min;
		...
		void callback(const sensor_msgs::LaserScan::ConstPtr& scan)
		{
			...
			left_min = minimun(210,269,k);  // 270 will be zero
		...
		void decision_(int j){	
			...
			if (stright_min <= 1.1 || left_min <= 0.57 || right_min <= 0.57){
				if (left_min<=0.5){
					decision_left = 1;
					decision_right= 0;
				}
			    ...
			    else if (left_min<right_min && left_min <0.57){
					decision_left = 1;
					decision_right= 0;
				}
				else if (left_min>right_min && right_min<0.57){
					decision_left = 0;
					decision_right= 1;
				}
			}
		    ...
		    else if(right_min < 0.57 && laser_temp[269] > 0.57 )// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 1;
				decision_left  = 0;
			
			}
			else if(left_min <0.57 && laser_temp[90]> 0.57)// && laser_temp[270] < 0.7 )//&& decision_right == 0 )
		    {
				decision_left = 1;
				decision_right= 0;
			...
			{	
				...
				//printf("%d\n",4);
				for(size_t i = 180; i < 270; i++) // 270 will be zero.
				...
				// wei need change
				if(decision_left ==1 && right_min < 0.3){
					read_1 += 0.1;
					read_2 += 0.1;
					read_3 += 0.1;
					read_4 += 0.1;
			...
			if(decision_right == 1)
			{
				...
				read_4 = laser_temp[40] ;
				read_5 = laser_temp[180] ;
				read_6 = laser_temp[65] ;

				for(size_t i = 150; i <180; i++)//25
				{	if(read_1>laser_temp[i])
					read_1=std::min(laser_temp[i],far);
				}
				//printf("%d\n",1);
				...
				// wei need change
				if(decision_right ==1 && left_min < 0.3){
					read_1 += 0.1;
					read_2 += 0.1;
					read_3 += 0.1;
					read_4 += 0.1;			
...
////////////////////////////////////  Output cmd_velocity  ////////////////////////////////////
		void test_run(int j){
			...
			if(decision_right == 1 ){
			 v1 = out_y[2] * radius * 1.0;
			 v2 = out_y[1] * radius * 1.0;
			}
